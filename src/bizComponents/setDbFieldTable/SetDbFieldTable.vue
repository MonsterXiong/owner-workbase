<template>
  <div class="common-page">
    <el-table :data="tableData" style="width: 100%" height="100%">
      ...
      <template v-if="columnConfig.isPrimaryKey">
      ...
      <template v-if="columnConfig.isQuery">
      ...
      <template v-if="columnConfig.isForm">
  ...
  methods: {
    initTemplate(data) {
      this.tableData = data.map(item => {
        ...
        if (this.columnConfig['isQuery']) {
          initParam.isQuery = isIncludeName
        }
        if (this.columnConfig['isForm']) {
          initParam.isForm = isIncludeName
          initParam.isRequired = !isAllowNull
        }
        // 是否删除
        if (this.columnConfig['isPrimaryKey']) {
          initParam.isPrimaryKey = isPrimaryKey
        }
        return initParam
      })
    },
