<template>
  <div class="matrix">
    ...
    >
      ...
      <TreeColumn :vertical="vertical" v-for="item in mainColumn" :data="item" :key="item.id">
        <template slot-scope="{ row, column, $index }">
          ...
          <span v-else-if="type === MatrixType.symmetric">&nbsp;<i :class="getData(row._id, column.property, row, column, $index)"></i>&nbsp;</span>
          <template v-else-if="type === MatrixType.input">
            <!-- {{ getData(row._id, column.property, column) }} -->
            <span @dblclick="changeInput(row._id, column.property)" v-show="row._id + '_' + column.property != isInput">{{
              inputData[row._id + '_' + column.property]
            }}</span>
            <el-input
              v-model="inputData[row._id + '_' + column.property]"
              v-show="row._id + '_' + column.property == isInput"
    ...
    <footer class="footer" v-if="type === MatrixType.relation">
      <div v-for="item in relationList" :key="item.value" :class="{ active: curRelation === item.value }" @click="onChangeRelation(item.value)">
        <i class="iconfont" :class="item.value"></i> {{ item.name }}
      </div>
      <div :class="{ active: curRelation === CLEARRELATION }" @click="onChangeRelation(CLEARRELATION)"><i class="el-icon-delete"></i>清除关系</div>
      <slot name="footer"></slot>
    </footer>
    <footer class="footer" v-if="type === MatrixType.symmetric">
      <div :class="{ active: curRelation === 'el-icon-back' }" @click="onChangeRelation('el-icon-back')">
        &nbsp;&nbsp; <i class="el-icon-back"></i> &nbsp;&nbsp;
      </div>
      <div :class="{ active: curRelation === 'el-icon-top' }" @click="onChangeRelation('el-icon-top')">
        &nbsp;&nbsp; <i class="el-icon-top"></i> &nbsp;&nbsp;
      </div>
      <div style="cursor: initial">B<i class="el-icon-back"></i>A<span>&nbsp;依赖关系:A依赖B </span></div>
      <div :class="{ active: curRelation === CLEARRELATION }" @click="onChangeRelation(CLEARRELATION)"><i class="el-icon-delete"></i>清除关系</div>
      <slot name="footer"></slot>
    </footer>
  </div>
