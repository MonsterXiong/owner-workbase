...
;(function (mod) {
  ...
  function javascriptHint(editor, options) {
    var ukeys = CodeMirror.ukeys // 获取用户自定义关键词
    return scriptHint(
      editor,
      javascriptKeywords,
      ukeys,
  ...
  var arrayProps = (
    'length concat join splice push pop shift unshift slice reverse sort indexOf ' + 'lastIndexOf every some filter forEach map reduce reduceRight '
  ).split(' ')
  ...
  function getCompletions(token, context, keywords, ukeys, options) {
    ...
    function gatherCompletions(obj) {
      ...
      forEach(ukeys, maybeAdd) // 匹配用户自定义关键词
